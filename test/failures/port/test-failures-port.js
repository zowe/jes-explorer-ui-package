...
const debug = require('debug')('test:explorer-ui-server:failures-port');
...
describe('test missing of configuration port entry', function() {
  ...
  it('should return exception and server shouldn\'t start', async function() {
    try {
      serverPid = await U.startTestServer('../test/failures/port/config.json');
      ...
      expect(e.output.stderr).to.include('port configuration is missing');
    }
  });
});
