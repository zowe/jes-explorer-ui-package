...
const debug = require('debug')('test:explorer-ui-server:failures-paths');
...
describe('test missing of configuration paths entry', function() {
  ...
  it('should return exception and server shouldn\'t start', async function() {
    try {
      serverPid = await U.startTestServer('../test/failures/paths/config.json');
      ...
      expect(e.output.stderr).to.include('paths configuration is missing');
    }
  });
});
