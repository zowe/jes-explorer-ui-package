...
const debug = require('debug')('test:explorer-ui-server:failures-certs');
...
describe('test missing of configuration certs entry', function() {
  ...
  it('should return exception and server shouldn\'t start', async function() {
    try {
      serverPid = await U.startTestServer('../test/failures/certs/config.json');
      ...
      expect(e.output.stderr).to.include('no such file or directory');
    }
  });
});
